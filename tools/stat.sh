...
TOKEN=`mysql -uroot -proot registry -s -N -e "select string_value from config where config_id='server_stat_token'";`
HOST=`mysql -uroot -proot registry -s -N -e "select string_value from config where config_id='server_host_port'";`
